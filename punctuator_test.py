...
def create_metriks_from_text(text):
    res_data = {}
    text = text.replace('-', ' ')
...
def create_capitalization_metriks(expected_cap_data, actual_cap_data):
    res_data = {}
    tp = 0
    fp = 0
    fn = 0
...
def calculate_punctuation(word_list):
    res_punctuation = {}
    res_no_punctuation = {}
...
def calculate_data_for_wer_report(expected_text, actual_text):
    report_data = {}
    report_data['nlp'] = {}
    report_data['stt'] = {}
    ...
    expected_data = create_metriks_from_text(expected_text)
    ...
    actual_data = create_metriks_from_text(actual_text)
    ...
    stt_wer = round(wer(report_data['stt']['actual'], report_data['stt']['expected']), 1)
    report_data['wer'] = stt_wer

    upper_cap_data = create_capitalization_metriks(expected_upper_case, actual_upper_case)
    ...
    lower_cap_data = create_capitalization_metriks(expected_lower_case, actual_lower_case)
    ...
    actual_punctuation, actual_no_punctuation = calculate_punctuation(actual_words)
    expected_punctuation, expected_no_punctuation = calculate_punctuation(expected_words)

    punctuation_data = create_capitalization_metriks(expected_punctuation, actual_punctuation)
    ...
    no_punctuation_data = create_capitalization_metriks(expected_no_punctuation, actual_no_punctuation)
